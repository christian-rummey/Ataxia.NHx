...
fit_slope_model <- function(data, group_var = "age_grp", include_all = TRUE) {
  data %>%
    ...
    map_dfr(function(dat) {
      ...
      age_groups <- unique(dat[[group_var]][!is.na(dat[[group_var]])])

      slope_predictions <- map_dfr(age_groups, function(grp) {
...
# age_groups        <- c( '16-24y', '25-40y' )  # Filter to specific age groups (NULL = all groups)
...
# pars. <- c('SARA','mFARS','USS','FARS.B','ADL', 'fane7')
...
# Prepare data for modeling (apply age group filter if specified)
dt_model <- dt %>%
  filter(paramcd %in% c('mFARS','USS','SARA','ADL'))

if (!is.null(age_groups)) {
  dt_model <- dt_model %>% filter(age_grp %in% age_groups)
}
