...
plot_cfb_by_group <- function(model_predictions, group_var = "age_grp",
                               colors = NULL, y_limits_list = NULL) {
  # Default color palette
  if (is.null(colors)) {
    colors <- c(
      "#E41A1C", # red
...
# Settings
include_all_group <- TRUE  # Set to FALSE to exclude "all" group from analysis
...
save_plot_to_ppt(
  plot_obj = p,
  title = "5y-Change from Baseline by Age Group",
  layout = "1s",
  i = 2
)
